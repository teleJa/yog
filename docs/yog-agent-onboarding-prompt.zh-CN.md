...
- Yog 的正常入口是 agent skill。插件安装完成并加载后，初始化、discover、review、promote、business-flow、sync、verify 都应通过 Yog skill 触发。
...
当 `yog` skill 已可用后，请使用 Yog 初始化当前仓库。

要求：

- 先确认当前仓库路径，并把它作为 `repoRoot`。
- `knowledgeRoot` 使用 `docs/knowledge`。
- 创建或更新 `docs/knowledge`、`.yog/config.json`、根 `AGENTS.md` / `CLAUDE.md` 的 Yog managed block。
- 不覆盖已有 `docs/knowledge/**` 文件；如发现已有文件被跳过，作为 P2 提示报告。
- 初始化后检查 `.yog/config.json`，确认 `knowledgeRoot`、`codeFactProvider` 字段合理。
- 初始化后运行 Yog `verify`；如果当前刚初始化导致索引为空或只有 P2 提示，请如实报告。
...
## 3. 询问是否执行 discover-candidates

初始化完成后，请停下来询问我是否现在执行 `discover-candidates`。
...
- 再进入 Yog `discover-candidates` workflow。
- 使用 3 个只读 subagent：`controller-route-agent`、`service-flow-agent`、`data-contract-agent`。
- 每个 subagent 设置 10-15 分钟 timeout。
- subagent 只输出候选建议，不能写 `docs/knowledge`。
- 主 agent 收集结果后走 `reduce-candidates.mjs`，gate 为 ok 后再用 `write-candidates.mjs` 写入候选。
- 完成后运行 `sync` 和 `verify`。

如果我不同意执行 discover：

- 停止在初始化完成状态。
- 给出后续可直接对 agent 说的自然语言请求，例如“使用 Yog discover 当前仓库的候选 context”。
