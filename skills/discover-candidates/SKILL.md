...
8. When reduce returns `gate: ok` or `gate: mid-low-scope-required` with exit code `0`, pass the reduce output to `write-candidates.mjs`. Stop before writing on input errors, blocking gates, or duplicate confirmation exits.
